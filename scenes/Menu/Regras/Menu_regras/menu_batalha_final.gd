...
func _ready() -> void: 
	for _button in get_tree().get_nodes_in_group("botoes_menu_regras_batalhaFinal"):
		_button.pressed.connect(_on_button_pressed.bind(_button))
		
func _input(event):
	if event.is_action_pressed("Esc"): 
		var _chance_scene: bool = get_tree().change_scene_to_file("res://Interface/Regras/Menu_regras/menu_principal_regras.tscn")
